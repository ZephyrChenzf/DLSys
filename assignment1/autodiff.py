...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        """TODO: Your code here"""
        assert len(input_vals)==2
        input1=input_vals[0].T if node.matmul_attr_trans_A else input_vals[0]
        input2=input_vals[1].T if node.matmul_attr_trans_B else input_vals[1]
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        if node in node_to_output_grad: continue
        ...
        for inx,inp in enumerate(node.inputs):
            if inp not in node_to_output_grads_list:
                node_to_output_grads_list[inp]=[]
            node_to_output_grads_list[inp].append(grads[inx])
