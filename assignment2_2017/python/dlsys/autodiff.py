...
class AddOp(Op):
    ...
    def infer_shape(self, node, input_shapes):
        ...
        if input_shapes[0]==input_shapes[1]:
            ...
            print(node.name +"infer shape error!")
            raise Exception
...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals, output_val, use_numpy=True):
        if use_numpy:
            ...
            input1=input_vals[0] if not node.matmul_attr_trans_A else np.transpose(input_vals[0])
            input2=input_vals[1] if not node.matmul_attr_trans_B else np.transpose(input_vals[1])
            output_val[:]=np.matmul(input1,input2)
        else:
    ...
    def gradient(self, node, output_grad):
        ...
        #     rhs_grad = matmul_op(
        #         output_grad, node.inputs[0], trans_A=True, trans_B=False)
        input1 = node.inputs[0] if not node.matmul_attr_trans_A else np.transpose(node.inputs[0])
        input2 = node.inputs[1] if not node.matmul_attr_trans_B else np.transpose(node.inputs[1])
        """
        Y=AB --- dA=dY B^T  --- dB=A^T dY
        """
        lhs_grad=matmul_op(output_grad,input2,trans_A=False,trans_B=True)
        rhs_grad=matmul_op(input1,output_grad,trans_A=True,trans_B=False)
    ...
    def memory_plan(self, feed_shapes):
        ...
        for node,shape in self.node_to_shape_map.items():
            if node in feed_shapes:
                continue
            newArr=None
            for memoryArr in self.memory_pool:
                if shape==memoryArr.shape:
                    newArr=memoryArr
                    self.memory_pool.remove(memoryArr)
                    break
            if newArr is None:
                newArr=ndarray.empty(shape,ctx=self.ctx)
