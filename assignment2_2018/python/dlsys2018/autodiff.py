...
class AddByConstOp(Op):
    ...
    def compiled_func(self, node, input_shapes, tgt, tgt_host):
        """TODO: Your code here"""
        return tvm_op.make_elemwise_add_by_const(input_shapes[0],node.const_attr,tgt,tgt_host,"elem_add_by_const")
...
class MulByConstOp(Op):
    ...
    def compiled_func(self, node, input_shapes, tgt, tgt_host):
        """TODO: Your code here"""
        return tvm_op.make_elemwise_mul_by_const(input_shapes[0],node.const_attr,tgt,tgt_host,"eelem_add_by_const")
...
class MatMulOp(Op):
    ...
    def infer_shape(self, node, input_shapes):
        """TODO: Your code here"""
        assert len(input_shapes)==2
        input1=input_shapes[0] if not node.matmul_attr_trans_A else (input_shapes[0][1],input_shapes[0][0])
        input2=input_shapes[1] if not node.matmul_attr_trans_B else (input_shapes[1][1],input_shapes[1][0])
        return (input1[0],input2[1])

    def compiled_func(self, node, input_shapes, tgt, tgt_host):
        """TODO: Your code here"""
        return tvm_op.make_matrix_mul(input_shapes[0],node.matmul_attr_trans_A,input_shapes[1],node.matmul_attr_trans_B,tgt,tgt_host,"mat_mul")
...
class BroadcastToOp(Op):
    ...
    def compiled_func(self, node, input_shapes, tgt, tgt_host):
        """TODO: Your code here"""
        return tvm_op.make_broadcast_to(input_shapes[0],broadcast_rule(input_shapes[0], input_shapes[1]),tgt,tgt_host,"broadcast_to")
...
class SoftmaxCrossEntropyOp(Op):
    ...
    def compiled_func(self, node, input_shapes, tgt, tgt_host):
        """TODO: Your code here"""
        return tvm_op.make_matrix_softmax_cross_entropy(input_shapes[0],tgt,tgt_host,"mat_softmax_cross_entropy")
...
class SoftmaxOp(Op):
    ...
    def compiled_func(self, node, input_shapes, tgt, tgt_host):
        """TODO: Your code here"""
        return tvm_op.make_matrix_softmax(input_shapes[0],tgt,tgt_host,"mat_softmax")
...
class Executor(object):
    ...
    def infer_shape(self, feed_shapes):
        ...
        for node in self.topo_order:
            ...
            for inp in node.inputs:
                input_shapes.append(self.node_to_shape_map[inp])
    ...
    def memory_plan(self, feed_shapes):
        ...
        if self.memory_pool is None:
            self.memory_pool=[]
        if self.node_to_arr_map is not None:
            self.memory_pool+=list(self.node_to_arr_map.values())
        ...
        for node,shape in self.node_to_shape_map.items():
            ...
            for item in self.memory_pool:
                if shape==item.shape:
                    ...
                    break
            if newArr is None:
                newArr=tvm.nd.empty(shape,ctx=self.ctx)
            self.node_to_arr_map[node]=newArr
